...
<head>
  ...
  <title>P.PHUKA HERB - เพิ่มข้อมูลวัตถุดิบ</title>
...
<body id="page-top">
    ...
    <div class="container pl-1">
      ...
      <div class="card shadow mb-4" width="800">
        <div class="card-header py-3">
          <h4 class="m-0 font-weight-bold text-primary">เพิ่มข้อมูลวัตถุดิบ</h4>
        </div>
        <div class="card-body">
          <div class="form-group">
            <form action="./process/material-add.php" method="post" enctype="multipart/form-data">
              <div class="form-group">
                <label class="m-0 font-weight-bold text-warning">ชื่อวัตถุดิบ</label>
                <input type="text" class="form-control form-control-user" id="exampleInputm_name" placeholder="ชื่อวัตถุดิบ" name="m_name" value="" required="">
              </div>
              <div class="form-group">
                <label class="m-0 font-weight-bold text-warning">รายละเอียดวัตถุดิบ</label>
                <input type="text" class="form-control form-control-user" id="exampleInputm_detail" placeholder="รายละเอียดสินค้า..." name="m_detail" value="">
              </div>
              <div class="form-group row">
                <div class="col-sm-6 mb-3 mb-sm-0">
                  <label class="m-0 font-weight-bold text-warning">จำนวน (กรัม)</label>
                  <input type="number" class="form-control form-control-user" id="examplem_no" placeholder="จำนวน" name="m_no" value="" required="">
                </div>
                <div class="col-sm-6">
  ...
  <script type="text/javascript">
   $("#cal-volume").click(function(){
    ...
    if(!Number.isNaN(packeting_no) && packeting_no!=0){
      for (var i = 0; i < parseInt(amount); i++) {
        var m_volume = parseInt($("#m_volume"+i).val());
        var mp_volume = m_volume/packeting_no;
        $("#mp_volume1"+i).val(mp_volume)
      }
    }
  });
   ...
   $("#cal-reset").click(function(){
    ...
    for (var i = 0; i < parseInt(amount); i++) {
      $("#mp_volume1"+i).val(tmp_mt_volume[i]);
    }
  }); 
</script>
</body>
