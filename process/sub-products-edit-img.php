...
$sql = "SELECT sp_img FROM sub_products WHERE sp_id = $sp_id"; 
$result = mysqli_query($conn,$sql);
if ($result->num_rows > 0) {
    $row = $result->fetch_assoc(); 
    $target_file2 = $target_dir.$row["sp_img"];                
...
if ($uploadOk == 3) {
	...
	$sql = "UPDATE sub_products SET sp_img = '$file_name' WHERE sp_id = '$sp_id'"; 
	$result = mysqli_query($conn,$sql);
	...
	if ($result) {
		echo "
		<script> 
		window.location.href ='../sub-products-edit.php?sp_id=$sp_id';
		</script>
		";
	}
	else{
