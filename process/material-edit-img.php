...
include './connect.php';
$target_dir = "../img/uploads/material/";
$target_file = $target_dir;
$m_id = htmlspecialchars(isset($_GET["m_id"])?$_GET["m_id"]:'');
$sql = "SELECT m_img FROM material WHERE m_id = $m_id"; 
$result = mysqli_query($conn,$sql);
if ($result->num_rows > 0) {
    $row = $result->fetch_assoc(); 
    $target_file2 = $target_dir.$row["m_img"];                
...
if ($uploadOk == 3) {
	...
	$sql = "UPDATE material SET m_img = '$file_name' WHERE m_id = '$m_id'"; 
	$result = mysqli_query($conn,$sql);
	...
	if ($result) {
		echo "
		<script> 
		window.location.href ='../material-edit.php?m_id=$m_id';
		</script>
		";
	}
	else{
