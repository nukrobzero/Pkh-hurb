    ...
    <div class="container pl-1">
      ...
      <div class="card shadow mb-4" width="800">
        ...
        <div class="card-body">
          ...
          $pdm_status= ISSET($_GET["pdm_status"])?$_GET["pdm_status"]:'';
          $user_status = $_SESSION["user_status"];

          ?>
          <div class="form-group">
            <form method="post" action="./process/products-manufacture-change-status.php?pdm_id=<?php echo $pdm_id; ?>&pdm_status=<?php echo $pdm_status; ?>&sp_id=<?php echo $sub_p_id; ?>">
                            ...
                            if ($result->num_rows > 0) {
                              ...
                              while ( $row = $result->fetch_assoc())
                              {
                                if($row["sp_id"]!=$pd_name)
                                  echo "<option value='".$row["sp_id"]."'>ราคา ( ".$row["sp_price"]." ) , ขนาดขวด ( ".$row["sp_volume"]." ซีซี)</option>";
                                else 
                                  echo "<option value='".$row["sp_id"]."' selected>ราคา ( ".$row["sp_price"]." ) , ขนาดขวด ( ".$row["sp_volume"]." ซีซี)</option>";
                              }
                              echo "</select>";   
                            } else {
                                      // echo $conn->error;
      ...
      // echo '<input type="hidden" name ="canclick" id="canclick" value="1">';
                   ...
                   <!--  <form id="products-manufacture-add">
                    ...
                    // $user_status
                    $button_status ="";
                    if ($pdm_status==1 or $pdm_status==99 or $pdm_status==100) {
                      $button_status = "disabled";
                    }
                    $show_confirm_button = '<button class="btn btn-primary btn-user btn-block" id ="add-manufacture1" name="pdm_status" value="1" type="submit" '.$button_status.'> 
                    อนุมัติรายการ
                    </button>'; 
                    ...
                    $show_edit_button = '<a href="products-manufacture-show_list_edit.php"><button class="btn btn-info btn-user btn-block" id ="add-manufacture1" name="pdm_status" value="100" type="submit" '.$button_status.'> 
                    แก้ไข
                    </button></a>';
                     //echo $user_status;
                    if ($user_status==99) {
                      echo $show_confirm_button."<hr>".$show_cancel_button;
                      //echo $show_confirm_button."<hr>".$show_edit_button."<hr>".$show_cancel_button;
                    }
                    else if ($user_status==98) {
                     echo $show_cancel_button."<hr>".$show_edit_button;
                   }
